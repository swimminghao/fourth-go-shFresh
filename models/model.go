...
type User struct { //用户表
	...
	Power     int          `orm:"default(0)"`     // 权限设置 0 未激活 1 已激活
...
type Address struct { //地址表
	...
	IsDefault bool   `orm:"defult(false)"` //是否默认 false 为非默认  true为默认
	//Active   bool   `orm:"defult(false)"`
...
type Goods struct { //商品SPU表
	...
	Detail   string      `orm:"size(20)"` //商品详情
	GoodsSKU []*GoodsSKU `orm:"reverse(many)"`
}

type GoodsType struct { //商品类型表
	Id                   int
	Name                 string                  `orm:"size(20)"` //种类名称
	Logo                 string                  `orm:"size(50)"` //logo
	Image                string                  `orm:"size(50)"` //图片
	GoodsSKU             []*GoodsSKU             `orm:"reverse(many)"`
...
type GoodsSKU struct { //商品SKU表
	...
	Desc                 string                  `orm:"size(200)"`  //商品介绍
	Price                int                     `orm:"default(0)"` //商品单价
	Unit                 string                  `orm:"size(20)"`   //商品单位
	Image                string                  //商品图片
...
type GoodsImage struct { //单个商品图片表
	Id       int
	Image    string
...
type OrderInfo struct { //订单表
	...
	PayMethod    int           `orm:"default(0)"` //支付方式
	TotalPrice   int           //总价格
	TotalCount   int           `orm:"default(1)""` //总数量
	TransitPrice int           //运费
...
func init() {
	...
	orm.RegisterDataBase("default", "mysql", "root:q123q123@tcp(10.211.55.5:3306)/dailyfresh?charset=utf8")
